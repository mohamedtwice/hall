...
import { getHonorees, getHonoreesByYear } from '@/lib/notion';
...
export default function Year({ honorees, year }) {
...
export async function getStaticPaths() {
    ...
    const paths = honorees.data.map((honoree) => ({
        params: { year: honoree.properties.Year.select.name},
    }));
    // console.log(paths)
...
export async function getStaticProps({ params }) {
    ...
    const honorees = await getHonoreesByYear(year);
    console.log(honorees)

    return { props: { honorees, year }, revalidate: 30 };
}
